...
namespace GestaoFacil.Server.Controllers.Financeiro
{
    ...
    public class CategoriaDespesaController : BaseController
    {
        ...
        private IQueryable<CategoriaDespesaModel> Visiveis() =>
            _context.CategoriasDespesa.Where(c => c.UsuarioId == null || c.UsuarioId == UsuarioId);

        //somente as proprias podem ser alteradas
        private IQueryable<CategoriaDespesaModel> Editaveis() =>
            _context.CategoriasDespesa.Where(c => c.UsuarioId == UsuarioId);
        ...
        public async Task<ActionResult<ResponseModel<CategoriaDto>>> Create(CategoriaCreateDto dto)
        {
            ...
            var entity = new CategoriaDespesaModel { Nome = nome, Ativo = true, UsuarioId = UsuarioId };
